...
export default function Home() {
    // use this to know whether a recipe modal should be open or not
    const [openRecipeModal, setOpenRecipeModal] = useState(false)

    // use this to choose which recipe to open
    const [recipeToOpenID, setRecipeToOpenID] = useState(0)

    function chooseRecipeToOpen(id) {
        setRecipeToOpenID(id)
        console.log(id)
    ...
    const recipeCards = [...Array(testData.length).keys()].map((i) => <button key={testData[i].id} onClick={() => chooseRecipeToOpen(i)}>{testData[i].title}</button>)

    return (
        <div className="home">
            
            {recipeCards}
            {openRecipeModal && <RecipeModal name={testData[recipeToOpenID].title} ingredients={testData[recipeToOpenID].image} closeModal={() => setOpenRecipeModal(false)}></RecipeModal>}
